#include "Scene.h"
#include "config.h"
#include "Scene_triangulation_3_color_item.h"
...
class Scene_intersection_color_item : public CGAL::Three::Scene_item_rendering_helper
{
  Q_OBJECT
public :
  Scene_intersection_color_item(Scene_triangulation_3_color_item* parent)
  :is_fast(false)
  {
    setParent(parent);
    alphaSlider = NULL;
  ...
  void init_vectors(
      ...
      std::vector<float> *p_faces_colors,
      std::vector<float> *p_vertices_colors,
      ...
      std::vector<float> *p_subdomain_ids)
  {
    ...
    normals = p_normals;
    edges = p_edges;
    faces_colors = p_faces_colors;
    vertices_colors = p_vertices_colors;
    barycenters = p_bary;
    subdomain_ids = p_subdomain_ids;
  }
  void setColor(QColor c) override
  {
    qobject_cast<Scene_triangulation_3_color_item*>(this->parent())->setColor(c);
  ...
  void computeElements() const override
  {
    ...
    getTriangleContainer(0)->allocate(Tc::FColors, faces_colors->data(),
                             static_cast<int>(faces_colors->size()*sizeof(float)));
    getTriangleContainer(0)->allocate(Tc::VColors, vertices_colors->data(),
                             static_cast<int>(vertices_colors->size()*sizeof(float)));
  ...
  void draw(CGAL::Three::Viewer_interface* viewer) const override
  {
    ...
    const EPICK::Plane_3& plane = qobject_cast<Scene_triangulation_3_color_item*>(this->parent())->plane();
    float shrink_factor = qobject_cast<Scene_triangulation_3_color_item*>(this->parent())->getShrinkFactor();
  ...
  void addTriangle(const TrInfo::Bare_point& pa, const TrInfo::Bare_point& pb,
                   const TrInfo::Bare_point& pc, const CGAL::IO::Color color)
  {
...
private:
  ...
  mutable std::vector<float> *faces_colors;
  mutable std::vector<float> *vertices_colors;
...
struct Scene_triangulation_3_color_item_priv {
  typedef CGAL::qglviewer::ManipulatedFrame ManipulatedFrame;
  Scene_triangulation_3_color_item_priv(Scene_triangulation_3_color_item* item)
  ...
  Scene_triangulation_3_color_item_priv(const TrInfo& triangulation_, Scene_triangulation_3_color_item* item)
  ...
  ~Scene_triangulation_3_color_item_priv()
  {
    if(alphaSlider)
      delete alphaSlider;
  ...
  void fill_aabb_tree() {
    ...
    timer.start();
    tree.clear();
    for (TrInfo::Finite_cells_iterator
  ...
  void draw_triangle(const TrInfo::Bare_point& pa,
                     const TrInfo::Bare_point& pb,
                     const TrInfo::Bare_point& pc) const;
  void draw_triangle_edges(const TrInfo::Bare_point& pa,
                           const TrInfo::Bare_point& pb,
                           const TrInfo::Bare_point& pc) const;
  ...
  Scene_triangulation_3_color_item* item;
  T3 triangulation;
  bool is_grid_shown;
  ...
  std::vector<TrInfo::Vertex> tr_vertices;
  Scene_intersection_color_item *intersection;
  ...
  std::array<std::bitset<32>, Scene_triangulation_3_color_item::number_of_bitset> visible_bitset;
...
struct Set_show_tetrahedra {
  Scene_triangulation_3_color_item_priv* priv;
  Set_show_tetrahedra(Scene_triangulation_3_color_item_priv* priv) : priv(priv) {}
...
void Scene_triangulation_3_color_item::common_constructor(bool display_elements)
{
...
Scene_triangulation_3_color_item::Scene_triangulation_3_color_item(bool display_elements)
  : Scene_group_item("unnamed")
  , d(new Scene_triangulation_3_color_item_priv(this))
{
  common_constructor(display_elements);
}

Scene_triangulation_3_color_item::Scene_triangulation_3_color_item(const T3 triangulation, bool display_elements)
  : Scene_group_item("unnamed")
  , d(new Scene_triangulation_3_color_item_priv(triangulation, this))
{
  common_constructor(display_elements);
  d->reset_cut_plane();
  triangulation_changed();
...
Scene_triangulation_3_color_item::~Scene_triangulation_3_color_item()
{
  if(d)
  {
    delete d;
...
const Scene_item*
Scene_triangulation_3_color_item::data_item() const
{
  return d->data_item_;
}

void
Scene_triangulation_3_color_item::set_data_item(const Scene_item* data_item)
{
...
void
Scene_triangulation_3_color_item::data_item_destroyed()
{
  set_data_item(NULL);
}

void
Scene_triangulation_3_color_item::check_new_item(int id)
{
...
void
Scene_triangulation_3_color_item::check_deleted_item(Scene_interface::Item_id id)
{
  if (d->is_item_clip_box(id))
...
const T3&
Scene_triangulation_3_color_item::triangulation() const {
  return d->triangulation;
}

T3&
Scene_triangulation_3_color_item::triangulation()
{
  return d->triangulation;
}

void
Scene_triangulation_3_color_item::changed()
{
  if(!d)
    return;
  d->need_changed = true;
...
void Scene_triangulation_3_color_item::updateCutPlane()
{
...
void
Scene_triangulation_3_color_item::triangulation_changed()
{
  // Update colors
  ...
  int max = 0;
  int compact = 0;
  for (TrInfo::Finite_cells_iterator cit = triangulation().finite_cells_begin(),
  ...
  for (TrInfo::Finite_facets_iterator fit = triangulation().finite_facets_begin(),
...
QPixmap
Scene_triangulation_3_color_item::graphicalToolTip() const
{
  if (!d->histogram_.isNull())
  {
    return d->histogram_;
  }
  const_cast<Scene_triangulation_3_color_item&>(*this).build_histogram();
  return d->histogram_;
}
...
create_histogram(const T3& triangulation, double& min_value, double& max_value)
{
    ...
    ++cit)
  {
    ...
    const TrInfo::Bare_point& p0 = wp2p(cit->vertex(0)->point());
    const TrInfo::Bare_point& p1 = wp2p(cit->vertex(1)->point());
    const TrInfo::Bare_point& p2 = wp2p(cit->vertex(2)->point());
    const TrInfo::Bare_point& p3 = wp2p(cit->vertex(3)->point());
...
void
Scene_triangulation_3_color_item::build_histogram()
{
...
QColor
Scene_triangulation_3_color_item::get_histogram_color(const double v) const
{
...
void
Scene_triangulation_3_color_item::update_histogram()
{
  build_histogram();
}

void
Scene_triangulation_3_color_item_priv::compute_color_map(const QColor& base_color)
{
...
Geom_traits::Plane_3 Scene_triangulation_3_color_item::plane(CGAL::qglviewer::Vec offset) const
{
...
void Scene_triangulation_3_color_item::compute_bbox() const {
  if (isEmpty())
    _bbox = Bbox();
  else {
    bool bbox_init = false;
    CGAL::Bbox_3 result;
    for (TrInfo::Finite_vertices_iterator
...
QString Scene_triangulation_3_color_item::toolTip() const {
...
void Scene_triangulation_3_color_item::draw(CGAL::Three::Viewer_interface* viewer) const {
  ...
  if(!visible())
    return;
  Scene_triangulation_3_color_item* ncthis = const_cast<Scene_triangulation_3_color_item*>(this);
  if(!isInit(viewer))
    initGL(viewer);
...
void Scene_triangulation_3_color_item::drawEdges(CGAL::Three::Viewer_interface* viewer) const {
  if(!visible())
    return;
...
void Scene_triangulation_3_color_item::drawPoints(CGAL::Three::Viewer_interface *) const
{

}

void Scene_triangulation_3_color_item_priv::draw_triangle(const TrInfo::Bare_point& pa,
                                         const TrInfo::Bare_point& pb,
                                         const TrInfo::Bare_point& pc) const
{
  Geom_traits::Vector_3 n = cross_product(pb - pa, pc - pa);
...
void Scene_triangulation_3_color_item_priv::draw_triangle_edges(const TrInfo::Bare_point& pa,
                                               const TrInfo::Bare_point& pb,
                                               const TrInfo::Bare_point& pc) const
{
  const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
...
double Scene_triangulation_3_color_item_priv::complex_diag() const {
...
QMenu* Scene_triangulation_3_color_item::contextMenu()
{
  const char* prop_name = "Menu modified by Scene_triangulation_3_color_item.";
  ...
  if (!menuChanged) {
    ...
    connect(d->tet_Slider, &QSlider::valueChanged, this, &Scene_triangulation_3_color_item::itemChanged);
...
void Scene_triangulation_3_color_item_priv::initializeBuffers(CGAL::Three::Viewer_interface *viewer)
{
  //vao containing the data for the facets
  {
    item->getTriangleContainer(Scene_triangulation_3_color_item::T3_faces)->initializeBuffers(viewer);
    item->getTriangleContainer(Scene_triangulation_3_color_item::T3_faces)->setFlatDataSize(
          positions_poly_size);
  ...
  //vao containing the data for the lines
  {
    item->getEdgeContainer(Scene_triangulation_3_color_item::T3_edges)->initializeBuffers(viewer);
    item->getEdgeContainer(Scene_triangulation_3_color_item::T3_edges)->setFlatDataSize(
  ...
  {
    item->getEdgeContainer(Scene_triangulation_3_color_item::Grid_edges)->initializeBuffers(viewer);
    item->getEdgeContainer(Scene_triangulation_3_color_item::Grid_edges)->setFlatDataSize(
          positions_grid.size());
  }
}
...
void Scene_triangulation_3_color_item_priv::computeIntersection(const Primitive& cell)
{
  Geom_traits::Construct_point_3 wp2p
    = item->triangulation().geom_traits().construct_point_3_object();

  typedef unsigned char UC;
  TrInfo::Cell_handle ch = cell.id();
  ...
  const TrInfo::Bare_point& pa = wp2p(ch->vertex(0)->point());
  const TrInfo::Bare_point& pb = wp2p(ch->vertex(1)->point());
  const TrInfo::Bare_point& pc = wp2p(ch->vertex(2)->point());
  const TrInfo::Bare_point& pd = wp2p(ch->vertex(3)->point());
...
struct ComputeIntersection {
  Scene_triangulation_3_color_item_priv& item_priv;

  ComputeIntersection(Scene_triangulation_3_color_item_priv& item_priv)
    : item_priv(item_priv)
  {}
...
void Scene_triangulation_3_color_item_priv::computeIntersections(CGAL::Three::Viewer_interface* viewer)
{
  const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
...
void Scene_triangulation_3_color_item_priv::computeSpheres()
{
  Geom_traits::Construct_point_3 wp2p
    = item->triangulation().geom_traits().construct_point_3_object();
      ...
      vit != end; ++vit)
  {
    ...
    typedef TrInfo::Vertex_handle Vertex_handle;
    ...
    const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
    TrInfo::Bare_point center(wp2p(vit->point()).x() + offset.x,
...
void Scene_triangulation_3_color_item_priv::computeElements()
{
  if(!alphaSlider)
   {
     alphaSlider = new QSlider(::Qt::Horizontal);
  ...
  //the facets
  {
    ...
    for (TrInfo::Finite_facets_iterator
         fit = item->triangulation().finite_facets_begin(),
         end = item->triangulation().finite_facets_end();
         fit != end; ++fit)
    {
      if(!item->do_take_facet(*fit)) continue;
      const TrInfo::Cell_handle& cell = fit->first;
      const TrInfo::Cell_handle& cell2 = cell->neighbor(fit->second);
      const int& index = fit->second;
      const TrInfo::Bare_point& pa = wp2p(cell->vertex((index + 1) & 3)->point());
      const TrInfo::Bare_point& pb = wp2p(cell->vertex((index + 2) & 3)->point());
      const TrInfo::Bare_point& pc = wp2p(cell->vertex((index + 3) & 3)->point());
...
bool Scene_triangulation_3_color_item::load_binary(std::istream& is)
{
  is >> triangulation();
  if(!is)
    return false;
...
void
Scene_triangulation_3_color_item_priv::reset_cut_plane()
{
  if(frame == 0)
...
void
Scene_triangulation_3_color_item::setColor(QColor c)
{
  color_ = c;
...
void
Scene_triangulation_3_color_item::setUseSubdomainColors(bool b)
{
  d->use_subdomain_colors = b;
}

void Scene_triangulation_3_color_item::show_grid(bool b)
{
  d->is_grid_shown = b;
...
void Scene_triangulation_3_color_item::show_intersection(bool b)
{
  contextMenu()->findChild<QAction*>("actionShowTets")->setChecked(b);
  if(b && !d->intersection)
  {
    d->intersection = new Scene_intersection_color_item(this);
...
void Scene_triangulation_3_color_item::reset_intersection_item()
{
  d->intersection = NULL;
}

void Scene_triangulation_3_color_item::reset_spheres()
{
  d->spheres = NULL;
}

CGAL::Three::Scene_item::ManipulatedFrame* Scene_triangulation_3_color_item::manipulatedFrame() {
  if(d)
    return d->frame;
  else
    return NULL;
...
void Scene_triangulation_3_color_item::setPosition(float x, float y, float z) {
   const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
  d->frame->setPosition(x+offset.x, y+offset.y, z+offset.z);
}

bool Scene_triangulation_3_color_item::has_spheres()const {
  return d->spheres_are_shown;
}

bool Scene_triangulation_3_color_item::has_grid()const { return d->is_grid_shown;}

bool Scene_triangulation_3_color_item::has_tets()const { return d->intersection; }

void Scene_triangulation_3_color_item::setNormal(float x, float y, float z) {
  d->frame->setOrientation(x, y, z, 0.f);
}

void Scene_triangulation_3_color_item::copyProperties(Scene_item *item)
{
  Scene_triangulation_3_color_item* t3_item = qobject_cast<Scene_triangulation_3_color_item*>(item);
  if(!t3_item)
    return;
   const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
...
float Scene_triangulation_3_color_item::getShrinkFactor() const
{
  return float(d->tet_Slider->value())/100.0f;
}

bool Scene_triangulation_3_color_item::eventFilter(QObject *, QEvent *event)
{
...
bool Scene_triangulation_3_color_item::keyPressEvent(QKeyEvent *event)
{
...
QString Scene_triangulation_3_color_item::computeStats(int type)
{
  Geom_traits::Construct_point_3 wp2p
    = triangulation().geom_traits().construct_point_3_object();
  ...
  switch (type)
  {
  case Scene_triangulation_3_color_item_priv::MIN_EDGES_LENGTH:
    return QString::number(d->min_edges_length);
  case Scene_triangulation_3_color_item_priv::MAX_EDGES_LENGTH:
    return QString::number(d->max_edges_length);
  case Scene_triangulation_3_color_item_priv::MEAN_EDGES_LENGTH:
    return QString::number(d->mean_edges_length);
  case Scene_triangulation_3_color_item_priv::MIN_DIHEDRAL_ANGLE:
    return QString::number(d->min_dihedral_angle);
  case Scene_triangulation_3_color_item_priv::MAX_DIHEDRAL_ANGLE:
    return QString::number(d->max_dihedral_angle);
  case Scene_triangulation_3_color_item_priv::MEAN_DIHEDRAL_ANGLE:
    return QString::number(d->mean_dihedral_angle);
  case Scene_triangulation_3_color_item_priv::NB_SPHERES:
    return QString::number(d->nb_spheres);
  case Scene_triangulation_3_color_item_priv::NB_VERTICES:
    return QString::number(d->nb_vertices);
  case Scene_triangulation_3_color_item_priv::NB_TETS:
    return QString::number(d->nb_tets);
  case Scene_triangulation_3_color_item_priv::SMALLEST_RAD_RAD:
    return QString::number(d->smallest_radius_radius);
  case Scene_triangulation_3_color_item_priv::SMALLEST_EDGE_RAD:
    return QString::number(d->smallest_edge_radius);
  case Scene_triangulation_3_color_item_priv::BIGGEST_VL3_CUBE:
    return QString::number(d->biggest_v_sma_cube);
  case Scene_triangulation_3_color_item_priv::NB_SUBDOMAINS:
...
CGAL::Three::Scene_item::Header_data Scene_triangulation_3_color_item::header() const
{
...
void Scene_triangulation_3_color_item::invalidateOpenGLBuffers()
{
...
void Scene_triangulation_3_color_item::resetCutPlane()
{
  if(!d)
    return;
  d->reset_cut_plane();
...
void Scene_triangulation_3_color_item::itemAboutToBeDestroyed(Scene_item *item)
{
...
void Scene_triangulation_3_color_item::on_spheres_color_changed()
{
  if(!d->spheres)
    return;
...
float Scene_triangulation_3_color_item::alpha() const
{
  if(!d->alphaSlider)
    return 1.0f;
...
void Scene_triangulation_3_color_item::setAlpha(int alpha)
{
  if(!d->alphaSlider)
    d->computeElements();
...
QSlider* Scene_triangulation_3_color_item::alphaSlider() {
  if(!d->alphaSlider)
    d->computeElements();
  return d->alphaSlider;
}

void Scene_triangulation_3_color_item::initializeBuffers(Viewer_interface *v) const
{
  const_cast<Scene_triangulation_3_color_item*>(this)->d->initializeBuffers(v);
}

void Scene_triangulation_3_color_item::computeElements()const
{
  QApplication::setOverrideCursor(Qt::WaitCursor);
  compute_bbox();

  const_cast<Scene_triangulation_3_color_item*>(this)->d->computeElements();
...
void Scene_triangulation_3_color_item::newViewer(Viewer_interface *viewer)
{
...
Scene_triangulation_3_color_item* Scene_triangulation_3_color_item::clone() const
{
  return new Scene_triangulation_3_color_item(d->triangulation);
}

void Scene_triangulation_3_color_item::show_spheres(bool b)
{
...
Scene_item::Bbox Scene_triangulation_3_color_item::bbox() const
{
  if(!is_bbox_computed)
    compute_bbox();
...
const std::set<int>& Scene_triangulation_3_color_item::subdomain_indices() const
{
  return d->subdomain_indices_;
}

QColor Scene_triangulation_3_color_item::getSubdomainIndexColor(int i) const
{
  return d->colors_subdomains[i];
}

void Scene_triangulation_3_color_item::resetVisibleSubdomain()
{
  d->visible_subdomain.set();
  d->visible_bitset.fill(0xFFFFFFFF);
}

void Scene_triangulation_3_color_item::switchVisibleSubdomain(int id)
{
...
bool Scene_triangulation_3_color_item::isVisibleSubdomain(int id) const
{
  return d->visible_subdomain[id];
}

void Scene_triangulation_3_color_item::computeIntersection()
{
  for(auto v : CGAL::QGLViewer::QGLViewerPool())
  {
...
void Scene_triangulation_3_color_item::set_cut_edge(bool b)
{
  d->cut_edges = b;
...
#include "Scene_triangulation_3_color_item.moc"
